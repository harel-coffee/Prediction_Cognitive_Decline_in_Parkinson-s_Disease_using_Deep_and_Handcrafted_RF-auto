...
# import mrmr
def RF(data, labels, nof,file,filename , X_External, Y_External):
    ...
    model = RandomForestClassifier(n_estimators=ne,criterion=cr,max_features=mf,    max_depth=md , min_samples_split=mss)
    # cvs = cross_val_score(model, data, labels, cv=nof)
    # print(cvs)
    # print(cvs.mean() , "\t" , cvs.std())
    
    r = ClassificationMetrics()
    r.set_predictorName("RandomForestClassifier")
    r.set_contin(False)
    r.set_DatasetName(filename)

    bestparameters = 'n_estimators= ' +str(ne) +'criterion= ' +str(cr) +'max_features= ' +str(mf) +'max_depth= ' +str(md) + ' min_samples_split= '+str(mss)
    ...
    for k_fe in K_F:
        ...
        print('*******************')
        MetricName = "RandomForestClassifier"
        ...
        for be in range (0,len(train_val)):
            ...
            r.set_foldNumber(str(fo)+'_'+str(k_fe))  
            r.Print("RandomForestClassifier")
    ...
    # clf = GridSearchCV(RandomForestClassifier(), params, n_jobs=-1, cv=nof )
    # clf.fit(data, labels)
    ...
    # model = RandomForestClassifier( max_depth=md , min_samples_split=mss)
