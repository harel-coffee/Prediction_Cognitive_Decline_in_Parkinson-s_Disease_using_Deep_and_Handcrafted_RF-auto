...
def LDA(data, labels, nof,file,filename ):
    params = {
        'solver': ['svd', 'lsqr', 'eigen'],
        'shrinkage': [None , 'auto' , 0.1 , 0.5 , 0.9],
    }
    clf = GridSearchCV(LinearDiscriminantAnalysis(), params, n_jobs=-1, cv=nof)
    clf.fit(data, labels)
    best_parameters = clf.best_params_
    sl = best_parameters['solver']
    sh = best_parameters['shrinkage']

    model = LinearDiscriminantAnalysis(solver=sl , shrinkage=sh)
    # cvs = cross_val_score(model, data, labels, cv=nof)
    # print(cvs)
    # print(cvs.mean() , "\t" , cvs.std())
    r = ClassificationMetrics()
    r.set_predictorName("LinearDiscriminantAnalysis")
    r.set_contin(False)
    r.set_DatasetName(filename)

    bestparameters = 'solver= ' +str(sl) + ' shrinkage= '+str(sh)
    ...
    r.Print("LinearDiscriminantAnalysis")
    r.Print("test")
