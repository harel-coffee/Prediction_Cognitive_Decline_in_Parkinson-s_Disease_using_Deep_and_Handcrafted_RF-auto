...
def PassiveAggressiveClass(data, labels,nof,file,filename ):
    params = {
        'C': [0.1, 0.5, 1, 5, 10, 50, 100],
    }
    clf = GridSearchCV(PassiveAggressiveClassifier(), params, n_jobs=-1, cv=nof)
    clf.fit(data, labels)
    best_parameters = clf.best_params_
    c = best_parameters['C']

    model = PassiveAggressiveClassifier(C=c )
    # cvs = cross_val_score(model, data, labels, cv=nof)
    # print(cvs)
    # print(cvs.mean() , "\t" , cvs.std())
    
    r = ClassificationMetrics()
    r.set_predictorName("PassiveAggressiveClassifier")
    r.set_contin(False)
    r.set_DatasetName(filename)

    bestparameters = 'C= ' +str(c)
    ...
    r.Print("PassiveAggressiveClassifier")
    r.Print("test")
