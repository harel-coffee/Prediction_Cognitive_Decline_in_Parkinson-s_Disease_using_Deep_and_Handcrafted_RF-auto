...
def adaboostclass(data, labels,nof,file,filename , X_External, Y_External):
    ...
    # print(results)
    model = AdaBoostClassifier(algorithm = 'SAMME',base_estimator=bs , n_estimators= ne , learning_rate= lr )
    r = ClassificationMetrics()
    r.set_predictorName("AdaBoostClassifier")
    r.set_contin(False)
    r.set_DatasetName(filename)
    bestparameters = 'base_estimator= ' +str(bs) + ' n_estimators= '+str(ne) + ' learning_rate= '+str(lr) 
    # bestparameters = 'base_estimator= ' +str(bs) + ' n_estimators= '+str(ne) + ' algorithm= '+str(al)
    ...
    # clf = GridSearchCV(AdaBoostClassifier(), params, n_jobs=-1, cv=nof )
    # clf.fit(data, labels)
    ...
    # model = AdaBoostClassifier(base_estimator=bs , n_estimators= ne , learning_rate= lr )
