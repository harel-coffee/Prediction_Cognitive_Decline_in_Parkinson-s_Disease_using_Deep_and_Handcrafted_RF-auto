...
# def rfc_cv(n_estimators, data, targets):
...
# def optimize_extra(data, targets):
...
# import mrmr
def Extra(data, labels, nof,file,filename , X_External, Y_External):



    # print(Colours.green("--- Optimizing Extra ---"))
    # optimize_extra(data, labels)
    ...
    # print("val. score: %s" % opt.best_score_)
    # print("test score: %s" % opt.score(X_External, Y_External))
    # print("best params: %s" % str(opt.best_params_))
    ...
    # clf = GridSearchCV(ExtraTreesClassifier(), params, n_jobs=-1, cv=kf)
    # clf.fit(data, labels)
    # best_parameters = clf.best_params_
    # mf = best_parameters['max_features']
    # ne = best_parameters['n_estimators']
    # cr = best_parameters['criterion']
    # # sp = best_parameters['splitter']
    ...
    model = ExtraTreesClassifier( bootstrap = True, class_weight=cw,max_depth=md,min_samples_split=mss,max_samples=ms, n_estimators= ne , max_features=mf , criterion=cr)
    # cvs = cross_val_score(model, data, labels, cv=nof)
    # print(cvs)
    # print(cvs.mean() , "\t" , cvs.std())
    r = ClassificationMetrics()
    r.set_predictorName("ExtraTreesClassifier")
    r.set_contin(False)
    r.set_DatasetName(filename)

    bestparameters = 'max_features= ' +str(mf) + ' n_estimators= '+str(ne)+ ' criterion= '+str(cr)
    ...
    for k_fe in K_F:
        ...
        print('*******************')
        MetricName = "ExtraTreesClassifier"
    ...
    # clf = GridSearchCV(ExtraTreesClassifier(), params, n_jobs=-1, cv=nof )
    # clf.fit(data, labels)
    # best_parameters = clf.best_params_
    # mf = best_parameters['max_features']
    # ne = best_parameters['n_estimators']
    ...
    # parameter_val.append(bestparameters)
    # model = ExtraTreesClassifier( n_estimators= ne , max_features=mf , criterion=cr)
