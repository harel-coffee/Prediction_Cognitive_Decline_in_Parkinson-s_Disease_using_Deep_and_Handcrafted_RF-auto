...
def XGB(data, labels, nof,file,filename ,X_External, Y_External ):
    ...
    bo = best_parameters['booster']
    md = best_parameters['max_depth']
    ml = best_parameters['max_leaf_nodes']
    mc = best_parameters['min_child_weight']
    ga = best_parameters['gamma']
    su = best_parameters['subsample']
    ms = best_parameters['max_delta_step']
    al = best_parameters['alpha']
    # la = best_parameters['lambda']
    ev = best_parameters['eval_metric']
    lr = best_parameters['learning_rate']
    ne = best_parameters['n_estimators']
    cb = best_parameters['colsample_bytree']
    ob = best_parameters['objective']
    sp = best_parameters['scale_pos_weight']
    ...
    # clf = GridSearchCV(KNeighborsClassifier(), params, n_jobs=-1, cv=kf)
    # clf.fit(data, labels)
    ...
    r.set_predictorName("XGBClassifier")
    r.set_contin(False)
    ...
    # clf = GridSearchCV(KNeighborsClassifier(), params, n_jobs=-1, cv=nof )
    # clf.fit(data, labels)
    # best_parameters = clf.best_params_
    # nn = best_parameters['n_neighbors']
    # ls = best_parameters['leaf_size']
    ...
    # bestparameters = 'n_neighbors= ' +str(nn) + ' leaf_size= '+str(ls)+ ' metric= '+str(me)
   
    # parameter_val.append(bestparameters)
    # model = KNeighborsClassifier(n_neighbors=nn , leaf_size=ls , metric=me)
