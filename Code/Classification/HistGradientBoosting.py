...
def histGradientBoost(data, labels, nof,file,filename, X_External, Y_External ):
    params = {
        'loss' : ['auto', 'binary_crossentropy', 'categorical_crossentropy'],
        'max_iter' : [50,100,200],
        'learning_ratefloat': [0.001,0.01 , 0.1 , 0.5 , 1],
        'l2_regularization': [0 , 0.01 ,0.01 ,0.2 ,0.3 , 0.5,0.8, 1]
    }
    kf = KFold(n_splits=nof,random_state=12,shuffle=True)
    clf = GridSearchCV(HistGradientBoostingClassifier(), params, n_jobs=-1, cv=kf)
    clf.fit(data, labels)
    best_parameters = clf.best_params_
    ls = best_parameters['loss']
    mi = best_parameters['max_iter']
    # es = best_parameters['early_stopping']
    # lr = best_parameters['learning_ratefloat']
    # l2 = best_parameters['l2_regularization']

    model = HistGradientBoostingClassifier( loss = ls , max_iter=mi )
    # cvs = cross_val_score(model, data, labels, cv=nof)
    # print(cvs)
    # print(cvs.mean() , "\t" , cvs.std())
    r = ClassificationMetrics()
    r.set_predictorName("HistGradientBoostingClassifier")
    r.set_contin(False)
    r.set_DatasetName(filename)

    bestparameters = 'loss= ' +str(ls) + ' max_iter= '+str(mi)
    ...
    for be in range (0,len(train_val)):
        ...
        r.set_foldNumber(str(fo)+'_'+str(k_fe))  
        r.Print("(HistGradientBoostingClassifier")
    ...
    clf = GridSearchCV(HistGradientBoostingClassifier(), params, n_jobs=-1, cv=nof )
    clf.fit(data, labels)
    best_parameters = clf.best_params_
    ls = best_parameters['loss']
    mi = best_parameters['max_iter']
