...
# import mrmr
def GradientBoosting(data, labels, nof,file,filename , X_External, Y_External):
    ...
    model = GradientBoostingClassifier(loss = ls , n_estimators=ne , criterion=cr ,max_features=mf )
    # model = GradientBoostingClassifier(loss = ls , n_estimators=ne , criterion=cr ,max_features=mf , max_depth=md)
    # cvs = cross_val_score(model, data, labels, cv=nof)
    # print(cvs)
    # print(cvs.mean() , "\t" , cvs.std())
    r = ClassificationMetrics()
    r.set_predictorName("GradientBoostingClassifier")
    r.set_contin(False)
    ...
    for k_fe in K_F:
        ...
        print('*******************')
        MetricName = "GradientBoostingClassifier"
        ...
        for be in range (0,len(train_val)):
            ...
            r.set_foldNumber(str(fo)+'_'+str(k_fe))  
            r.Print("GradientBoostingClassifier")
    ...
    # clf = GridSearchCV(GradientBoostingClassifier(), params, n_jobs=-1, cv=nof )
    # clf.fit(data, labels)
