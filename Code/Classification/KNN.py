...
def knn(data, labels, nof,file,filename ,X_External, Y_External ):
    ...
    opt = BayesSearchCV(
        KNeighborsClassifier(),
        {
            "n_neighbors": (1, 51),
            "leaf_size": (5, 65),
            "metric": Categorical(['euclidean', 'manhattan', 'cityblock' , 'minkowski']),
    ...
    model = KNeighborsClassifier(algorithm=al, n_neighbors=nn , leaf_size=ls , metric=me)
    # cvs = cross_val_score(model, data, labels, cv=nof)
    # print(cvs)
    # print(cvs.mean() , "\t" , cvs.std())
    
    r = ClassificationMetrics()
    r.set_predictorName("KNeighborsClassifier")
    r.set_contin(False)
    r.set_DatasetName(filename)

    bestparameters = 'algorithm= ' +str(al) +'n_neighbors= ' +str(nn) + ' leaf_size= '+str(ls)+ ' metric= '+str(me)
    ...
    for k_fe in K_F:
        ...
        print('*******************')
        MetricName = "KNeighborsClassifier"
