...
def mlp(data, labels, nof,file,filename, X_External, Y_External ):
    ...
    bs = best_parameters['batch_size']



    model = MLPClassifier( batch_size=bs, solver=sol  ,activation=act , alpha=alph ,
    learning_rate = lr , momentum = mom )
    # cvs = cross_val_score(model, data, labels, cv=nof)
    # print(cvs)
    # print(cvs.mean() , "\t" , cvs.std())
    r = ClassificationMetrics()
    r.set_predictorName("MLP")
    r.set_contin(False)
    ...
    for k_fe in K_F:
        ...
        fo = 1
        r = ClassificationMetrics()
        r.set_predictorName("MLPClassifier")
        r.set_DatasetName(filename)
    ...
    # clf = GridSearchCV(MLPClassifier(), params, n_jobs=-1, cv=nof )
    # clf.fit(data, labels)
    ...
    # parameter_val.append(bestparameters)
    # model = MLPClassifier(solver=sol  ,activation=act , alpha=alph ,
    # learning_rate = lr , momentum = mom )
